...
export const intercomService = {
    ...
    async start(env, {user, orm}) {
        ...
        const isAsteriskAdmin = await user.hasGroup('asterisk_plus.group_asterisk_admin')

        if (isAsteriskAdmin) {
            const self = this
            self.support_data = await orm.call('asterisk_plus.settings', 'get_instance_support_data')
    ...
    manageIntercom(url) {
        ...
        if (url.includes('model=asterisk_plus')) {
            ...
            setTimeout(() => {
                if (window.odooPbxSupport) return
